-- ================================================================
-- Fix founding_explorer cutoff to match actual GeoRanks 2.0 launch
-- 2026-06-08
--
-- Problem: FOUNDING_CUTOFF was hardcoded to '2026-05-23 00:00:00+00'
-- (the date the Phase 3D achievements migration was authored locally),
-- but the GeoRanks 2.0 achievements system did not actually go live
-- in production until commit 127ae9d deployed via Vercel on
-- 2026-06-07 21:25:33 +01:00 BST (= 2026-06-07 20:25:33 UTC).
--
-- Effect of the old cutoff:
--   - Already safe against over-award: no post-launch signup could
--     ever satisfy created_at < '2026-05-23', so nobody who joined
--     after launch could receive founding_explorer.
--   - But it under-awarded: real early adopters who created their
--     account between 2026-05-23 and the actual launch were excluded,
--     even though they "were here before achievements existed" (the
--     badge's own description) — achievements simply weren't live yet.
--
-- Fix: replace FOUNDING_CUTOFF with the real production launch
-- timestamp, 2026-06-07 20:25:33+00. This is a drop-in CREATE OR
-- REPLACE of evaluate_and_award_achievements() — the ONLY change is
-- the FOUNDING_CUTOFF constant value (and its explanatory comment).
-- No other achievement logic, condition, threshold, or query changes.
--
-- This is forward-looking only:
--   - Does not touch existing user_achievements rows — nobody loses
--     a badge they already correctly hold.
--   - Pre-launch users who were wrongly excluded will be awarded
--     founding_explorer the next time evaluate_and_award_achievements()
--     runs for them (login / gameplay), via the normal
--     ON CONFLICT DO NOTHING + IF FOUND pipeline — no backfill needed.
--   - Post-launch users still cannot qualify: created_at for any
--     account created after 2026-06-07 20:25:33 UTC fails
--     created_at < FOUNDING_CUTOFF, exactly as before.
-- ================================================================
