-- ================================================================
-- Historical count fallback for game/duel achievements
-- 2026-06-08
--
-- Problem: bronze_first_steps, bronze_getting_started, silver_quarter_century,
-- gold_centurion, platinum_georanks_legend, bronze_first_blood,
-- silver_duel_veteran, and gold_duel_master all key off v_games_count /
-- v_duel_count, both of which are sourced exclusively from game_sessions —
-- a table created 2026-05-22 that has never been written to (the INSERT
-- code ships in this same release). Every existing user would therefore
-- start these achievements at zero, despite having extensive play history
-- in the legacy best-score tables.
--
-- Fix: wrap each count in GREATEST(live, historical_estimate). The
-- historical estimate is a conservative LOWER BOUND derived by summing
-- distinct best-score rows across the legacy tables — it can only
-- under-count true plays (a user may have replayed a category many times
-- but only their best run is retained), never over-count, so this can
-- never award a badge nobody earned.
--
-- Self-obsoleting by design: once game_sessions accumulates enough real
-- rows to exceed the frozen historical estimate, GREATEST() naturally
-- prefers the live count and the fallback becomes inert. No follow-up
-- migration will be needed to "turn it off".
--
-- No schema changes. No new RPC. ON CONFLICT DO NOTHING + IF FOUND +
-- v_newly_awarded are untouched — the existing award/dedup/popup-queue
-- pipeline handles historically-qualifying badges exactly like live ones.
-- ================================================================
